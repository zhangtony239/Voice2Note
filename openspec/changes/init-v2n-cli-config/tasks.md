...
- [x] 9.6 密集语音单元切分转写：实测混入长静音的解码单元会重复幻觉（与时长无关），改为语音区检测（容忍 <0.4s 停顿）+ 边缘 0.15s 呼吸空间 + 句间停顿 ≤0.5s 才合并 + <1s 微单元并入 + 超 30s 音量感知保底硬切；验证：真实录音 10 分钟切 65 单元（平均 8.3s、全部 ≤30s），此前幻觉区域全部恢复真实内容、零垃圾单元
- [x] 9.7 解码循环抑制：`generate` 加 `repetition_penalty=1.2`（解码层面修复）；A/B 验证："性别零零零…"×512 token 循环单元 dup 1.00→0.04 且恢复真实内容，英文基准句与正常中文单元回归质量不变
