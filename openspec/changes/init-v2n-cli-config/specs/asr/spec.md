...
### Requirement: 长音频分块转写且切点音量感知
超过模型单次处理上限（audio tower 位置嵌入 2048 帧，约 122 秒）的音频 SHALL 分块转写后拼接：每块不超过单次上限；切点 SHALL 在目标位置之前做音量感知（选取最安静的窗口），避免切断句子。

#### Scenario: 长音频分块
- **WHEN** 音频时长超过单次处理上限（如 45 分钟录音）
- **THEN** 音频被分为多块分别转写，拼接为一份 STT 原稿，不发生张量尺寸错误

#### Scenario: 切点避开句子中间
- **WHEN** 目标切点附近存在静音段
- **THEN** 切点落在静音处（目标位置之前最安静的窗口），而不是句子中间
