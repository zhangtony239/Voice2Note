...
**Reason**: CLI 入口改为 `v2n START-END "TITLE"`：TITLE 为必填位置参数，同时承担原 PROMPT 的「随原稿发送给 LLM」与「决定笔记文件名」职责，可选 PROMPT 用法不再存在。
...
`v2n` SHALL 接受两个必填位置参数：`START-END`（音频文件编号范围）与 `TITLE`（笔记标题）。执行顺序 SHALL 为：加载配置 → 按 `START-END` 选取音频文件 → 对范围内每个文件执行 ASR 转写并在 `TRANSCRIPT_PATH` 下保存 STT 原稿 → 全部转写完成后将所有原稿一次性发送给 LLM → 在 `NOTE_PATH` 下生成一篇笔记 markdown。缺少任一位置参数时 SHALL 输出用法错误并以非零退出码退出，不开始处理任何音频文件。
...
#### Scenario: 缺少位置参数
- **WHEN** 用户执行 `v2n`（无参数）或 `v2n 141-142`（缺少 TITLE）
...
`v2n` SHALL 在范围内全部文件的 STT 原稿均保存完成后，才调用 LLM；LLM SHALL 只被调用一次，接收合并了全部原稿的一份文档，并只产出一篇笔记。范围内任一文件 ASR 失败时，v2n SHALL 向 stderr 输出错误信息并以非零退出码退出，不调用 LLM。
